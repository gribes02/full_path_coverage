...
from common_msgs.msg import Cell
...
class MapDrive(MarkerVisualization):
	def __init__(self):
		...
		rospy.Subscriber("cell_to_cover", Cell, self.cellReceived, queue_size=1)
		...
		self.finished_path_coverage_pub = rospy.Publisher("finished_path_coverage", Bool, queue_size=1)
	...
	def cellReceived(self, cell):
		if self.global_costmap is None or self.local_costmap is None:
			rospy.logerr("No global or local costmap, doing nothing.")
			return
		
		self.global_frame = cell.frame_id
		points = [(p.x, p.y) for p in cell.points]
		self.visualize_area(points, close=True) # visualize area to cover on the clients rviz
...
if __name__ == "__main__":
	...
	while not rospy.is_shutdown():
		if p.finished_coverage:
			p.finished_path_coverage_pub.publish(Bool(True))
		rate.sleep()
